<template>
    ...
    <div class="row">
      ...
      <div class="col-6 d-flex justify-content-end">
        <div class="input-group" style="max-width: 400px">
          ...
          <input type="search" name="search" id="search" v-model="searchTextTmp" class="form-control" />
      ...
      <table class="table table-striped table-hover">
        ...
        <tbody>
          <tr v-for="(product, index) in products" :key="index">
            ...
            <td>
              <img :src="`${this.API_URL}/product/photo/${product.productId}`" width="100" height="100" />
            </td>
            <td>{{ product.productName }}</td>
            <td>
              <span v-if="product.productStatus == 1" class="badge bg-success">上架中</span>
            ...
            <td>
              <!-- 操作按鈕 -->
              <button type="button" class="btn btn-primary" @click="editProduct(product.productId)">
                修改
              </button>
            </td>
            <td>
              <button type="button" class="btn btn-danger" @click="turnOffProduct(product.productId)"
                v-if="product.productStatus == 1">
                下架
              </button>

              <button type="button" class="btn btn-success" @click="turnOnProduct(product.productId)" v-else>
                上架
              </button>
            </td>
            <td>
              <button type="button" class="btn btn-warning" @click="deleteProduct(product.productId)">
                刪除
              </button>
            </td>
          </tr>
  ...
  <main v-if="showEditPage" class="container">
    ...
    <div class="mb-3">
      ...
      <input type="text" class="form-control" v-model="results.productName" required />
    </div>

    <div class="mb-3" v-if="results.productId">
      <label for="productPhoto" class="form-label"><b>現有商品圖片預覽</b></label>
      <img :src="`${this.API_URL}/product/photo/${results.productId}`" width="150" height="150" />
    </div>

    <div class="mb-3">
      <label for="productPhoto" class="form-label"><b>更改商品頁面圖片</b></label>
      <input type="file" accept=".jpg,.png" id="productPhoto" ref="productPhoto" class="form-control" />
    </div>
    ...
    <div class="row">
      <div class="col-md-3">
        <div class="mb-3">
          <label for="select1" class="form-label"><b>第一層類別(如不修改類別可不選)</b></label>
          <select class="form-select" id="select1" v-model="firstCategoryId" @change="filterSecondChildCategories()">
      ...
      <div class="col-md-3" v-if="firstCategoryId && secondCategories && secondCategories.length > 0
    ">
        <div class="mb-3">
          <label for="select2" class="form-label"><b>第二層類別</b></label>
          <select class="form-select" id="select2" v-model="secondCategoryId" @change="filterThirdChildCategories()">
            <option selected disabled>請選擇</option>
            <option v-for="sc in secondCategories" :value="sc.productCategoryId">
              {{ sc.categoryName }}
            </option>
          </select>
        </div>
      </div>
      <div class="col-md-3" v-if="secondCategoryId && thirdCategories && thirdCategories.length > 0">
        <div class="mb-3">
          <label for="select3" class="form-label"><b>第三層類別</b></label>
          <select class="form-select" id="select3" v-model="thirdCategoryId" @change="filterFourthChildCategories()">
            <option selected disabled>請選擇</option>
      ...
      <div class="col-md-3" v-if="thirdCategoryId && fourthCategories && fourthCategories.length > 0
    ">
        <div class="mb-3">
          <label for="select4" class="form-label"><b>第四層類別</b></label>
          <select class="form-select" id="select4" v-model="fourthCategoryId">
            <option selected disabled>請選擇</option>
            <option v-for="fc in fourthCategories" :value="fc.productCategoryId">
              {{ fc.categoryName }}
            </option>
          </select>
        </div>
    ...
    <div class="mb-3">
      <template v-for="(ps, index) in results.productSpecDtos" :key="index">
        <div class="row g-2 mt-2">
          <div class="col">
            <input type="text" class="form-control" v-model="ps.productSpecName" placeholder="產品規格名稱" required />
          </div>
          <div class="col">
            <input type="number" class="form-control" v-model="ps.stockQuantity" placeholder="產品規格庫存" required />
          </div>
          <div class="col">
            <input type="number" class="form-control" v-model="ps.productSellingPrice" placeholder="產品規格實價" required />
          </div>
          <div class="col">
            <input type="number" class="form-control" v-model="ps.productOriginPrice" placeholder="產品規格售價" required />
          </div>
          <div class="col-auto align-self-end">
            <button type="button" class="btn btn-danger" @click="deleteProductSpec(index)">
              刪除
            </button>
          </div>
        </div>
      </template>
      <button type="button" class="btn btn-primary mt-2" @click="addProductSpec">
        新增商品規格
      </button>
    </div>

    <div class="mb-3">
      <label for="productDescription" class="form-label"><b>商品描述:</b></label>

      <div style="border: 1px solid #ccc; margin-top: 10px">
        <Toolbar :editor="editorRef" :defaultConfig="toolbarConfig" :mode="mode"
          style="border-bottom: 1px solid #ccc" />
        <Editor :defaultConfig="editorConfig" :mode="mode" v-model="results.productDescription"
    ...
    <div class="mb-3">
      <input type="reset" class="btn btn-secondary me-2" value="取消變更" @click="cancelEdit" />
